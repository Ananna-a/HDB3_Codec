...
module can_top #(
        ...
        input  wire [28:0] tx_id,     // 🔥 V2.8: CAN ID (29-bit, 支持标准帧和扩展帧)
        input  wire        tx_ide,    // 🔥 V2.8: ID扩展位 (0=标准帧11位, 1=扩展帧29位)
        input  wire [ 3:0] tx_len,    // Data length (DLC: 0-8 bytes)
        input  wire [63:0] tx_data,   // the data to push to tx fifo (up to 8 bytes)
    ...
    reg         buff_valid = 1'b0;
    reg         buff_ready = 1'b0;
    // buff_data, buff_id, buff_ide, buff_len 在下方wire声明

    reg         pkt_txing = 1'b0;
    reg  [28:0] pkt_tx_id = 29'h0;      // 🔥 V2.8: 扩展到29位
    reg         pkt_tx_ide = 1'b0;     // 🔥 V2.8: IDE标志
    reg  [ 3:0] pkt_tx_len = 4'd0;
    ...
    // ---------------------------------------------------------------------------------------------------------------------------------------
    //  TX buffer (🔥 V2.8扩展为98位: 29位ID + 1位IDE + 4位DLC + 64位数据)
    // ---------------------------------------------------------------------------------------------------------------------------------------
    localparam DSIZE = 98;
    localparam ASIZE = 10;
    ...
    always @ (posedge clk)
        if(tx_valid & ~full)
            buffer[wptr[ASIZE-1:0]] <= {tx_id, tx_ide, tx_len, tx_data};  // 🔥 V2.8: {29位ID, 1位IDE, 4位DLC, 64位数据}
    ...
    // 🔥 V2.8: 解析29位ID + IDE标志
    wire [28:0] buff_id = rdack ? rddata[97:69] : keepdata[97:69];
    wire        buff_ide = rdack ? rddata[68] : keepdata[68];
    wire [ 3:0] buff_len = rdack ? rddata[67:64] : keepdata[67:64];
    wire [63:0] buff_data = rdack ? rddata[63:0] : keepdata[63:0];
    ...
    always @ (posedge clk or negedge rstn)
        if(~rstn) begin
            ...
            if(r_cnt>4'd0) begin             // send the data bytes out
            ...
            else if(pkt_rx_valid) begin  // recieve a new packet
                ...
                else if(~pkt_rx_ide) begin                                                                   // is a short-ID data packet
                ...
                else begin                                                                                   // is a long-ID data packet
            ...
            if(~pkt_txing) begin
                ...
                if(t_rtr_req | buff_valid) begin     // if recieved a remote packet, or tx-buffer data available
                    ...
                    if(buff_valid) begin             // update data from tx-buffer , if tx-buffer is available
                        pkt_tx_id <= buff_id;
                        pkt_tx_ide <= buff_ide;     // 🔥 V2.8: 读取IDE标志
                        pkt_tx_len <= buff_len;
