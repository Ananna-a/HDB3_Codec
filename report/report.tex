...
\begin{itemize}
  \item 输入 bit 为 1 时，若 \texttt{zero\_cnt} 不为零，先进入 \texttt{S\_FLUSH} 状态将暂存零逐一输出，之后返回 \texttt{S\_READ} 再处理该 bit 1；若无暂存零，则按 AMI 规则直接输出普通正脉冲 \texttt{+1} 或普通负脉冲 \texttt{-1}，翻转 \texttt{ami\_pol}，	exttt{pulse\_parity} 取反，\texttt{zero\_cnt} 归零。
...
进入 \texttt{S\_SUBST} 后，编码器根据 \texttt{pulse\_parity} 判断替换形式：若为偶数，采用 \texttt{B00V} 替换，补信脉冲 \texttt{B} 的极性遵循 AMI 交替规则，破坏脉冲 \texttt{V} 的极性与 \texttt{B} 相同（故意破坏交替规律）；若为奇数，采用 \texttt{000V} 替换，破坏脉冲 \texttt{V} 的极性取上一次破坏脉冲的相反值。替换完成后，将四个替换符号顺序写入 4 级移位缓存 \texttt{subst\_buf}，由状态机依次输出，同时清零 \texttt{zero\_cnt} 和 \texttt{pulse\_parity}，更新 \texttt{last\_pol} 和 \texttt{ami\_pol}，返回 \texttt{S\_RUN} 继续处理。
...
HDB3 规则在不同教材中描述方式差异较大。常见的一种叙述是：首次遇到四连零时固定采用 \texttt{000V}，后续则根据两次替换组之间的 1 个数奇偶性判断；另一种叙述（本设计采用的奇偶法）则是统一按自上一次破坏脉冲以来普通 1 的奇偶个数判断，初始奇偶状态为偶数。
...
示波器上 DA0 与 DA1 的波形在宽度和形状上不同，这是在两个层次上有意做出的区分。从信号含义层看，DA0 承载的是 HDB3 编码后的符号序列（包含正负普通脉冲、破坏脉冲和补信脉冲），DA1 承载的是译码恢复后的二进制 bit 序列。从输出形式层看，DA0 采用双极性归零（RZ）格式——正电平表示正符号、负电平表示负符号、零电平表示零符号，每个脉冲仅占据半个码元以清晰区分相邻符号；DA1 采用单极性不归零（NRZ）格式——高电平表示 bit 1、零电平表示 bit 0，每个 bit 保持完整码元以与原始数字信号形式一致。

两路波形的差异恰恰体现了 HDB3 编译码前后的信号变换：DA0 展示编码如何用正负脉冲和替换脉冲表示原始数据，DA1 展示译码如何将编码符号恢复为原始二进制形式。因此，观察波形时应关注码元边界对齐和译码结果的正确性，而不是要求两路波形形状相同。
