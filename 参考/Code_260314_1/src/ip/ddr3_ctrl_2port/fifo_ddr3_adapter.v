    ...
    always @(posedge ui_clk or negedge rst_n) begin
        if(~rst_n) begin
            ...
            case(state)
                ...
                WRITE:  begin
                    // 🔥 V8.7.41修复: 在写状态也检查wr_rst，立即重置地址
                    if(wr_rst) begin
                        state <= DDR3_DONE;
                        wr_addr_cnt <= 24'd0;
                        app_addr_wr <= app_addr_wr_min_a;
                    end
                    else if((wr_addr_cnt == (wr_bust_len_a - 1)) && (app_rdy && app_wdf_rdy)) begin
                        state <= DDR3_DONE;
                ...
                READ: begin
                    // 🔥 V8.7.41修复: 在读状态也检查wr_rst，立即重置地址
                    if(wr_rst) begin
                        state <= DDR3_DONE;
                        wr_addr_cnt <= 24'd0;
                        app_addr_wr <= app_addr_wr_min_a;
                    end
                    else if((rd_addr_cnt == (rd_bust_len_a - 1)) && app_rdy ) begin
                        state <= DDR3_DONE;
