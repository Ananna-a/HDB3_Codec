...
系统设计中，HDB3 编码器按照 AMI 极性交替和四连零替换规则生成 \texttt{+1}、\texttt{-1}、\texttt{+V}、\texttt{-V}、\texttt{+B}、\texttt{-B} 等符号；译码器将普通正负脉冲恢复为 1，将零电平、破坏脉冲和补信脉冲恢复为 0。FPGA 通过 \texttt{55 AA} 命令帧接收上位机请求，通过 \texttt{AA 55} 应答帧返回处理结果。DA0 输出 HDB3 双极性归零波形，DA1 输出译码后的 NRZ 波形，便于示波器对比观察。

实验部分完成了独立仿真 RTL 结构检查、上位机与 FPGA 编码测试、上位机与 FPGA 译码测试以及示波器实测验证。全 0、全 1、10 交替、典型混合、长 0、编码随机和解码随机等测试结果表明，FPGA 返回结果与上位机软件参考结果一致，示波器波形能够体现 HDB3 码的正负交替、四连零替换和译码恢复特性。实验验证了本系统 HDB3 编译码功能的正确性。
...
其中，\texttt{V} 是破坏脉冲，其极性故意破坏 AMI 极性交替规律，使接收端能够识别四连零替换位置；\texttt{B} 是补信脉冲，用于维持整体直流平衡。替换完成后，编码器重新开始统计普通 1 的奇偶状态。
...
本设计的译码器采用直接符号映射方式，不再反向查找 \texttt{B00V} 或 \texttt{000V} 的组合位置。这种方式与本系统采用的编码符号定义一致，便于 FPGA 批处理实现和上位机逐项比较。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{c c X}
    ...
    DAC 通道 & DA0 / DA1 & DA0 输出 HDB3 RZ 波形，DA1 输出译码 NRZ 波形 \\
    \bottomrule
  \end{tabularx}
\end{table}

该结构将数字比对和波形观察结合起来。上位机比对能够精确判断每一个符号或 bit 是否正确；示波器波形能够直观展示 HDB3 码对长连零的处理效果和译码恢复结果。
...
DA0 用于输出 HDB3 编码波形，DA1 用于输出译码后的二进制波形。DA0 采用双极性归零形式：正符号输出正电平，负符号输出负电平，后半个码元回到零电平；DA1 采用 NRZ 形式：bit 为 1 时输出高电平，bit 为 0 时输出零电平。两路 DAC 共用采样时钟，因此码元边界对齐。

\begin{figure}[H]
  \centering
  \begin{tikzpicture}[x=0.82cm,y=0.82cm,font=\small]
    \foreach \x in {0,1,...,10}{
      \draw[wavegrid] (\x,-1.5) -- (\x,2.2);
      \node[font=\scriptsize,gray!70] at (\x+0.5,-1.35) {\x};
    }
    \draw[->] (0,-1.15) -- (10.8,-1.15) node[right]{码元时间};
    \node[anchor=east] at (-0.82,1.55) {DA1 译码 NRZ};
    \node[anchor=east] at (-0.82,0.15) {DA0 HDB3 RZ};

    \draw[thick,blue!70!black] (0,1.95) -- (1,1.95) -- (1,1.15) -- (2,1.15) -- (2,1.95) -- (5,1.95) -- (5,1.15) -- (7,1.15) -- (7,1.95) -- (10,1.95);
    \draw[thick,red!70!black] (0,0.15) -- (0,0.55) -- (0.5,0.55) -- (0.5,0.15) -- (1.5,0.15) -- (1.5,-0.25) -- (2,-0.25) -- (2,0.15) -- (4,0.15) -- (4,0.55) -- (4.5,0.55) -- (4.5,0.15) -- (7,0.15) -- (7,-0.25) -- (7.5,-0.25) -- (7.5,0.15) -- (10,0.15);
    \node[font=\scriptsize,blue!70!black] at (9.2,2.23) {整码元保持};
    \node[font=\scriptsize,red!70!black] at (8.7,0.78) {半码元归零};
  \end{tikzpicture}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{实物连线图.png}
  \caption{实物连接关系}
...
FPGA 编码器以批处理方式读取顶层缓存中的 bit 数据。模块内部维护连续零计数、当前 AMI 极性以及自上一次破坏脉冲以来普通 1 的奇偶状态。输入 bit 为 1 时，编码器按 AMI 规则输出普通正脉冲或普通负脉冲，并翻转下一次普通脉冲极性；输入 bit 为 0 时，编码器累加连续零个数。当连续零个数达到 4 时，根据奇偶状态输出 \texttt{B00V} 或 \texttt{000V}。

编码结果一方面写入应答缓存，后续通过 UART 返回上位机；另一方面写入 DA0 波形缓存，用于示波器显示。编码模式下，系统还会把编码结果送入译码器完成回环译码，使 DA1 同步输出译码恢复波形。

\subsection{HDB3 译码器实现}

译码器按符号逐项处理输入 HDB3 序列。符号为 \texttt{+1} 或 \texttt{-1} 时输出 bit 1；符号为 \texttt{0}、\texttt{+V}、\texttt{-V}、\texttt{+B} 或 \texttt{-B} 时输出 bit 0。该实现与表 \ref{tab:symbol_definition} 中的符号定义一致，逻辑简单，便于 FPGA 稳定实现。

在译码模式下，上位机输入 HDB3 符号序列，FPGA 将输入符号写入 DA0 缓存，同时将译码输出写入 DA1 缓存和应答缓存。这样示波器可以同时观察“输入 HDB3 波形”和“译码 bit 波形”。

\subsection{主控状态机实现}

顶层状态机负责完成一次命令的完整生命周期。其流程包括等待命令、解析载荷、启动编码或译码、写入 DAC 缓存、启动 DAC 播放和发送 UART 应答。状态机流程如图 \ref{fig:main_fsm} 所示。
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[font=\small]
    ...
    \node[smallblock] (idle_done) at (19.3,0.10) {回到空闲\\\texttt{M\_IDLE}};
    ...
    \draw[arrow] (send.east) -- node[above]{完成} (idle_done.west);

    \draw[returnbus] (idle.south) -- ++(0,-2.30) -- (err.west);
    \node[font=\scriptsize,anchor=south] at (4.20,-2.28) {帧错误 / 参数错误};
    \draw[returnbus] (err.east) -- ++(5.20,0) -| (send.south);
  \end{tikzpicture}
  }
  \caption{顶层主控状态机流程}
  \label{fig:main_fsm}
...
为了说明报告中的系统功能确实对应到工程实现，本文选取 FPGA 编码核心、DAC 播放核心和上位机命令发送核心三处代码进行摘录。代码只保留关键路径，完整实现见工程中的 \texttt{hdb3\_encoder.v}、\texttt{dac\_playback.v} 和 \texttt{MainViewModel.cs}。
...
上位机采用 C\# WPF 编写，界面分为编码测试区和译码测试区。编码测试区输入二进制序列，显示软件 HDB3 编码结果和 FPGA 返回结果；译码测试区输入 HDB3 符号序列，显示软件译码结果和 FPGA 返回结果。上位机通过颜色和文本状态显示比对是否一致。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{上位机界面.png}
  \caption{上位机测试界面}
  \label{fig:pc_interface}
\end{figure}

软件部分主要包括 \texttt{HDB3Codec.cs}、\texttt{SerialService.cs} 和 \texttt{MainViewModel.cs}。其中 \texttt{HDB3Codec.cs} 实现软件参考编译码算法；\texttt{SerialService.cs} 负责串口收发和帧解析；\texttt{MainViewModel.cs} 负责界面绑定、命令发送和结果比对。

\section{仿真与实测验证}

\subsection{独立仿真工程验证}

图 \ref{fig:rtl_sim} 为独立仿真工程生成的 RTL 结构图。该图来自 ModelSim/Vivado 仿真参考工程，用于说明 HDB3 编码器、译码器与 testbench 的独立仿真结构。它不是最终 Gowin 工程的系统级 RTL 图，因此本文仅将其作为仿真补充材料，不用于说明最终 FPGA 系统模块划分。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{仿真的RTL框图.png}
  \caption{独立仿真工程 RTL 结构图}
  \label{fig:rtl_sim}
\end{figure}

从独立仿真结构可以看出，测试环境主要围绕 HDB3 编码模块、译码模块和测试激励展开，用于检查编码输出和译码恢复是否满足基本逻辑要求。该仿真顶层为 \texttt{hdb3\_top}，由 \texttt{tb\_hdb3} 提供时钟、复位和输入序列。仿真输入序列为 \texttt{1000010000111100}，其中包含四连零、普通 1 和连续 1，便于同时观察 HDB3 替换、极性交替和译码恢复。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.95\textwidth]{Modelsim仿真图.png}
  \caption{ModelSim HDB3 编译码波形仿真}
  \label{fig:modelsim_wave}
\end{figure}

图 \ref{fig:modelsim_wave} 中各端口的关系如表 \ref{tab:modelsim_ports} 所示。仿真时钟 \texttt{clk} 按半码元周期翻转，顶层内部用 \texttt{phase} 产生 \texttt{bit\_tick}，因此每两个 \texttt{clk} 周期对应一个完整码元。\texttt{data\_in} 是输入的单极性二进制序列；编码器输出的 \texttt{hdb3\_sym\_pos}、\texttt{hdb3\_sym\_neg} 和 \texttt{hdb3\_valid} 表示完整码元的 HDB3 符号，并直接送入译码器；\texttt{hdb3\_pos} 和 \texttt{hdb3\_neg} 则由顶层进行半码元门控，形成便于观察的双极性归零波形。译码输出由 \texttt{data\_out} 和 \texttt{data\_valid} 给出，\texttt{code\_error} 用于标记非法的双线同时有效状态。
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{>{\ttfamily}c c X}
    ...
    hdb3\_pos / hdb3\_neg & 输出观测 & 顶层根据 \texttt{phase} 门控后的归零脉冲波形 \\
...
全 0 序列中没有普通 1，如果采用普通 AMI 码会长时间保持零电平。HDB3 编码会周期性插入替换脉冲，从而避免长连零。图 \ref{fig:all_zero_test} 中，上位机显示 FPGA 返回结果与软件参考结果一致；示波器上 DA0 出现替换脉冲，DA1 保持低电平，说明译码结果恢复为 0。
...
全 1 序列不触发四连零替换，主要验证 AMI 极性交替规则。图 \ref{fig:all_one_test} 中，FPGA 返回结果与软件参考一致；示波器上 DA0 呈现连续正负交替脉冲，DA1 保持高电平，符合全 1 序列译码结果。
...
10 交替序列用于验证普通 1 的正负交替和 0 的零电平间隔。该序列不产生四连零替换，因此 DA0 中应当表现为间隔出现的正负脉冲，DA1 中应当表现为 1、0 交替的 NRZ 波形。测试结果如图 \ref{fig:alternate_test} 所示。
...
典型混合序列同时包含普通 1、连续 0 和连续 1，是最能综合体现 HDB3 编码规则的测试用例。图 \ref{fig:mixed_test} 中，上位机比对结果一致；示波器波形中既能观察到 AMI 正负交替，也能观察到四连零替换产生的脉冲，DA1 能够恢复原始输入节奏。
...
长 0 序列用于验证多组连续四零替换。图 \ref{fig:long_zero_test} 中，FPGA 返回结果与软件参考一致；示波器上 DA0 能看到多组替换脉冲，说明 HDB3 编码有效抑制了长连零。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{译码可直接导入-以全1为例.png}
  \caption{编码结果导入译码区示例}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{cX X c}
    ...
    测试项目 & 上位机比对 & 示波器观察 & 结论 \\
    \midrule
    全 0 编码 & 软件结果与 FPGA 返回一致 & DA0 出现替换脉冲，DA1 为低电平 & 通过 \\
    全 1 编码 & 软件结果与 FPGA 返回一致 & DA0 正负交替，DA1 为高电平 & 通过 \\
    10 交替编码 & 软件结果与 FPGA 返回一致 & DA0 间隔脉冲，DA1 交替输出 & 通过 \\
    典型序列编码 & 软件结果与 FPGA 返回一致 & 普通脉冲和替换脉冲均可观察 & 通过 \\
...
独立仿真工程能够观察 HDB3 编码和译码逻辑，但它不包含最终系统中的 UART 协议、上位机交互和 DAC 播放结构。因此，本文没有把独立仿真 RTL 图作为最终 FPGA 模块框图，而是单独放在仿真验证部分。最终系统正确性主要依据上位机比对和示波器实测结果。

\subsection{四连零替换规则的统一}

HDB3 规则在教材中常以破坏脉冲和补信脉冲的直观作用进行讲解，而工程实现中更适合采用奇偶法直接判断 \texttt{B00V} 或 \texttt{000V}。为保证软硬件结果一致，本设计以上位机 \texttt{HDB3Codec.cs} 和 FPGA \texttt{hdb3\_encoder.v} 中采用的奇偶法为统一依据。

\subsection{DA0 与 DA1 波形差异}

示波器上 DA0 与 DA1 的波形宽度不同，这是由输出形式决定的。DA0 是 HDB3 双极性归零波形，脉冲只占据半个码元；DA1 是译码 bit 的 NRZ 波形，一个 bit 保持完整码元。因此判断波形时应关注码元边界和译码结果，而不是要求两路波形形状完全相同。

\section{总结}

本文完成了一套基于 FPGA 的 HDB3 编译码器系统。系统实现了二进制序列到 HDB3 符号序列的编码、HDB3 符号序列到二进制序列的译码、上位机串口控制、FPGA 应答返回和双通道 DAC 波形输出。与单纯软件仿真相比，本设计能够同时给出软件参考结果、FPGA 实际返回结果和示波器实测波形，验证链路更加完整。

实验结果表明，在全 0、全 1、10 交替、典型混合、长 0、编码随机和解码随机等工况下，上位机软件参考结果与 FPGA 返回结果一致；示波器能够观察到 HDB3 码的正负交替脉冲、四连零替换脉冲和译码恢复波形。由此说明 FPGA 编译码逻辑、UART 通信和 DAC 输出均达到设计要求。

后续可进一步扩展序列长度，补充覆盖最终 Gowin 顶层的系统级 testbench，并增加更长时间随机序列测试和异常帧处理测试，使系统验证更加充分。
